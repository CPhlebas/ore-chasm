/*
    ...
    $Id: CVar.h 164 2010-03-01 20:19:13Z gsibley $
...
template <typename T> std::ostream& operator<<(std::ostream &out, const CVarRef<T> &v)
{
    out << *v.var;
    return out;
}
template <typename T> std::istream& operator>>(std::istream &in, CVarRef<T> &v)
{
    in >> *v.var;
    return in;
}
